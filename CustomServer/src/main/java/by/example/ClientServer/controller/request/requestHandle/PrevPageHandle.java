...
import by.example.ClientServer.controller.request.impl.PrevPageRequest;
...
public class PrevPageHandle implements RequestHandler{
    @Override
    public Response executeRequest(Request request, ServerThread server) {
        TableController.tuneMaxRecordsNum(server.getTableParameters(), ((PrevPageRequest) request).getMaxRecordsNum());
        TableController.decrementPage(server.getTableParameters());
        TableController.tuneTableParameters(server.getTableParameters(), server.getRepository().getStudentList().size());
        Server.logger.info("Prev page on");
